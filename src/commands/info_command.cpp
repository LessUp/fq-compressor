...
[[nodiscard]] std::string_view codecFamilyToString(CodecFamily family) noexcept {
    switch (family) {
        case CodecFamily::kRaw:         return "Raw";
        case CodecFamily::kAbcV1:       return "ABC-v1";
        case CodecFamily::kScmV1:       return "SCM-v1";
        case CodecFamily::kDeltaLzma:   return "Delta+LZMA";
        case CodecFamily::kDeltaZstd:   return "Delta+Zstd";
        case CodecFamily::kDeltaVarint: return "Delta+Varint";
        case CodecFamily::kOverlapV1:   return "Overlap-v1";
        case CodecFamily::kZstdPlain:   return "Zstd";
        case CodecFamily::kScmOrder1:   return "SCM-Order1";
        case CodecFamily::kExternal:    return "External";
        case CodecFamily::kReserved:    return "Reserved";
    }
    return "unknown";
}
...
[[nodiscard]] std::string codecByteToString(std::uint8_t codec) {
    ...
    if (ver > 0) oss << "." << static_cast<int>(ver);
    return oss.str();
}
...
void InfoCommand::printTextInfo() {
    ...
    std::cout << "Size:             " << fileSize << " bytes (" << formatBytes(fileSize) << ")" << std::endl;
    ...
    std::cout << "Compression algo: " << codecFamilyToString(static_cast<CodecFamily>(gh.compressionAlgo)) << std::endl;
    std::cout << "Checksum type:    " << checksumTypeToString(static_cast<ChecksumType>(gh.checksumType)) << std::endl;
    std::cout << "Quality mode:     " << qualityModeToString(format::getQualityMode(gh.flags)) << std::endl;
    std::cout << "ID mode:          " << idModeToString(format::getIdMode(gh.flags)) << std::endl;
    std::cout << "Read length class: " << readLengthClassToString(format::getReadLengthClass(gh.flags)) << std::endl;
    std::cout << "Paired-end:       " << (format::isPaired(gh.flags) ? "yes" : "no") << std::endl;
    if (format::isPaired(gh.flags)) {
        std::cout << "PE layout:        " << peLayoutToString(format::getPeLayout(gh.flags)) << std::endl;
    }
    std::cout << "Preserve order:   " << (format::isPreserveOrder(gh.flags) ? "yes" : "no") << std::endl;
    std::cout << "Has reorder map:  " << (format::hasReorderMap(gh.flags) ? "yes" : "no") << std::endl;
    std::cout << "Streaming mode:   " << (format::isStreamingMode(gh.flags) ? "yes" : "no") << std::endl;
    ...
    if (options_.showIndex || options_.detailed) {
        ...
        std::cout << "Global checksum:  0x" << std::hex << ft.globalChecksum << std::dec << std::endl;

        for (std::size_t i = 0; i < index.size(); ++i) {
            const auto& e = index[i];
            std::cout << "  Block " << std::setw(4) << i
                      << "  offset=" << e.offset
                      << "  size=" << e.compressedSize
                      << "  reads=" << e.readCount
                      << "  range=[" << e.archiveIdStart << "," << e.archiveIdEnd() << ")"
                      << std::endl;
        }
    }
...
void InfoCommand::printJsonInfo() {
    ...
    std::cout << "    \"major\": " << static_cast<int>(format::decodeMajorVersion(version)) << "," << std::endl;
    std::cout << "    \"minor\": " << static_cast<int>(format::decodeMinorVersion(version)) << std::endl;
    std::cout << "  }," << std::endl;

    // Global header
    std::cout << "  \"global_header\": {" << std::endl;
    std::cout << "    \"total_reads\": " << gh.totalReadCount << "," << std::endl;
    std::cout << "    \"compression_algo\": \"" << codecFamilyToString(static_cast<CodecFamily>(gh.compressionAlgo)) << "\"," << std::endl;
    std::cout << "    \"checksum_type\": \"" << checksumTypeToString(static_cast<ChecksumType>(gh.checksumType)) << "\"," << std::endl;
    std::cout << "    \"quality_mode\": \"" << qualityModeToString(format::getQualityMode(gh.flags)) << "\"," << std::endl;
    std::cout << "    \"id_mode\": \"" << idModeToString(format::getIdMode(gh.flags)) << "\"," << std::endl;
    std::cout << "    \"read_length_class\": \"" << readLengthClassToString(format::getReadLengthClass(gh.flags)) << "\"," << std::endl;
    std::cout << "    \"paired\": " << (format::isPaired(gh.flags) ? "true" : "false") << "," << std::endl;
    if (format::isPaired(gh.flags)) {
        std::cout << "    \"pe_layout\": \"" << peLayoutToString(format::getPeLayout(gh.flags)) << "\"," << std::endl;
    }
    std::cout << "    \"preserve_order\": " << (format::isPreserveOrder(gh.flags) ? "true" : "false") << "," << std::endl;
    std::cout << "    \"has_reorder_map\": " << (format::hasReorderMap(gh.flags) ? "true" : "false") << "," << std::endl;
    std::cout << "    \"streaming_mode\": " << (format::isStreamingMode(gh.flags) ? "true" : "false") << "," << std::endl;
    std::cout << "    \"original_filename\": \"" << jsonEscape(reader.originalFilename()) << "\"," << std::endl;
    ...
    std::cout << "    \"global_checksum\": \"0x" << std::hex << ft.globalChecksum << std::dec << "\"" << std::endl;
    std::cout << "  }," << std::endl;
...
void InfoCommand::printBlockDetails() {
    ...
    for (BlockId bid = 0; bid < static_cast<BlockId>(blockCount); ++bid) {
        ...
        std::cout << "  Uniform length: " << (bh.hasUniformLength() ? std::to_string(bh.uniformReadLength) : "variable") << std::endl;
        std::cout << "  Compressed:     " << bh.compressedSize << " bytes (" << formatBytes(bh.compressedSize) << ")" << std::endl;
        std::cout << "  Checksum:       0x" << std::hex << bh.blockXxhash64 << std::dec << std::endl;
        ...
        std::cout << "  Stream sizes:   IDs=" << bh.sizeIds
                  << "  Seq=" << bh.sizeSeq
                  << "  Qual=" << bh.sizeQual
                  << "  Aux=" << bh.sizeAux << std::endl;
...
void InfoCommand::printCodecStats(format::FQCReader& reader) {
    ...
    auto printStreamCodecs = [](const char* name, const std::map<std::string, std::uint64_t>& codecs, std::uint64_t total) {
        ...
        for (const auto& [codec, bytes] : codecs) {
            ...
            std::cout << "    " << codec << ": " << formatBytes(bytes)
