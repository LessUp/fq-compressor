...
本文件记录 fq-compressor 项目的所有重要变更，按日期倒序排列。

格式基于 [Keep a Changelog](https://keepachangelog.com/)，遵循 [Conventional Commits](https://www.conventionalcommits.org/)。

---

## [2026-03-08] — CI 质量门禁修复

### Fixed
- 全项目 62 个源文件 clang-format 格式化，消除所有格式违规
- `.gitignore` 移除 `package-lock.json` 排除项，修复文档 CI `npm ci` 失败
- `scripts/lint.sh` 拆分 `find_sources` 为 `find_all_sources`（格式检查）和 `find_lint_sources`（静态分析），排除测试文件避免 `rapidcheck.h` not found 错误
- `docs-pages.yml` / `docs-quality.yml` 路径触发器增加 `package-lock.json`

### Changed
- `.claude/index.json` 全面更新：项目状态从 `design_phase` 更新为 `implementation`，修正文件计数和模块列表
- `CLAUDE.md` 更新：模块进度、测试数量、依赖列表、项目状态全面同步
...
## [2026-01-18] — DevContainer/Docker 同步
